...
def create_overview_charts(df, title="Overview"):
    fig = make_subplots(rows=2, cols=2, subplot_titles=('Temperature', 'Voltage', 'Capacitance', 'Current'))
    ...
    fig.update_layout(height=600, title=title, plot_bgcolor='#1e1e1e', paper_bgcolor='#1e1e1e', font=dict(color='white'))
    return fig
...
with tab2:
    if st.session_state.current_experiment:
        ...
        st.plotly_chart(create_overview_charts(df, title=f"{st.session_state.experiment_name} Overview"), use_container_width=True)
    else:
